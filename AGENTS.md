...
| `examples/mea/`                 | Unpublished CLI: discover, turn, capture, doctor, digests (the smoke and drift tool)                                    |
| `examples/hub-host/`            | Unpublished reference host: the retry, reconciliation and stream-ownership loop over an injected Hub port               |
...
- **Vendor fixtures are captured by `mea capture`, never hand-edited.** In a capture directory that
  ...
  The archival JSONL transcripts and `claude/help/` carry no manifest and are not covered; see
  `fixtures/README.md`. `mea digests` rewrites the digests from the committed files without running
  a vendor CLI. Public contracts are regenerated at the pinned vendor versions. A labelled historical capture stays fixed: it records
...
- Conventional Commits with a body: `type(scope): summary`. Scopes: `core`, `claude`, `codex`,
  `acp`, `mea`, `examples`, `fixtures`, `docs`, `ci`, `build`, `deps`, `release`. `mea` is the
  smoke CLI specifically; `examples` is any other unpublished crate under `examples/`. One concern
  per commit.
- A pull request title is squashed into the subject that git-cliff reads, so it obeys the same
  rule. `scripts/check-pr-title.sh` enforces it, reading the scopes from the bullet above and the
  types from the parsers in `cliff.toml`; the `PR title` workflow runs the base branch's copy of
  that script, so a new scope has to land before a title can use it.
